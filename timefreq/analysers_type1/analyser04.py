...
def create_train_sample(random_state=None, noise_ratio=None):
    ...
    freq_domain_result = np.zeros(N_CLASSES)
    freq_domain_result[freq_idx-2: freq_idx+3] = np.array([0.01, 0.04, 0.9, 0.04, 0.01])

    return signal_y, freq, freq_domain_result
...
if __name__ == '__main__':
    ...
    # first layer
    model = tf.add(tf.matmul(fft, weights['w1']), biases['b1'])
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(N_TRAINING_EPOCHS):
            ...
            for _ in range(TRAIN_SIZE):
                signal_y, freq, net_result = create_train_sample(random_state, 0.002)
