...
# THIS IS ANALYSER13: same as 10, 2 more layers
...
N_HIDDEN2 = int(1024 * INC)
N_HIDDEN3 = int(1024 * INC)
N_HIDDEN4 = int(1024 * INC)
N_HIDDEN5 = int(4048 * INC)
...
def generate_signal(freq, random_state, noise_ratio=None):
    ...
    for current_freq in chosen_harmonics:
        ...
        amplitude = np.exp(np.log(0.01) + random_state.rand() * (np.log(1.0) - np.log(0.01)))
        signal_y += amplitude * np.sin((TWO_PI * current_freq) * signal_x + start_phase)
...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'w4': tf.Variable(tf.random_normal([N_HIDDEN3, N_HIDDEN4], 0, 0.01)),
        'w5': tf.Variable(tf.random_normal([N_HIDDEN4, N_HIDDEN5], 0, 0.01)),
        'out': tf.Variable(tf.random_normal([N_HIDDEN5, N_CLASSES], 0, 0.001)),
    }
    biases = {
        ...
        'b4': tf.Variable(tf.random_normal([N_HIDDEN4], 0, 0.001)),
        'b5': tf.Variable(tf.random_normal([N_HIDDEN5], 0, 0.001)),
    ...
    with tf.Session() as sess:
        ...
        for i_epoch in range(N_TRAINING_EPOCHS):
            ...
            if i_epoch % 1 == 0:
                train_data_directory_path = '/home/dankal/bazy/audio/fake/f0analysis_ver3_noise0_0005'
            ...
            if i_epoch % 1 == 0:
                ...
                # if i_epoch % 50 == 49:
                #     stop = 1
                #     comparison_array = np.vstack([dev_true_freqs, predicted_freqs])
                #     indexes_of_errors = np.where(np.abs(np.diff(comparison_array, axis=0)) > 3)[1]
                #     error_array = comparison_array[:, indexes_of_errors].T
                #     error_signals = np.array(dev_in)[indexes_of_errors]
                #
                #     for idx_error in range(len(error_signals)):
                #         import matplotlib.pyplot as plt
                #         fig, axes = plt.subplots(2)
                #         axes[0].plot(error_signals[0])
                #         axes[1].inshow(np.fft.fft(error_signals[0]))
                #         plt.title(error_array[:, idx_error])
                #
                #         fig, axes = plt.subplots(2)
                #         axes[0].plot(error_signals[0])
                #         spectrum = np.fft.fft(error_signals[0])
                #         freq_domain = np.linspace(0, FS, len(spectrum))
                #         half_nfft = int(len(spectrum) / 2)
                #         axes[1].plot(freq_domain[0:half_nfft], np.abs(spectrum[0:half_nfft]))
                #         axes[0].set_title(error_array[idx_error, :])
            stop = 1
