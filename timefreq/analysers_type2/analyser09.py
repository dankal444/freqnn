...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'w1': tf.Variable(tf.random_normal([len(sine_sdt_matrix) * 4, N_HIDDEN1], 0, 0.0001)),
        'w2': tf.Variable(tf.random_normal([N_HIDDEN1, N_HIDDEN2], 0, 0.001)),
        'w3': tf.Variable(tf.random_normal([N_HIDDEN2, N_HIDDEN3], 0, 0.001)),
        'out': tf.Variable(tf.random_normal([N_HIDDEN3, N_CLASSES], 0, 0.0001)),
    }
    biases = {
        'b1': tf.Variable(tf.random_normal([N_HIDDEN1], 0, 0.0001)),
        'b2': tf.Variable(tf.random_normal([N_HIDDEN2], 0, 0.0001)),
        'b3': tf.Variable(tf.random_normal([N_HIDDEN3], 0, 0.0001)),
        'out': tf.Variable(tf.random_normal([N_CLASSES], 0, 0.00001)),
    }
    ...
    concatenated_sdt = tf.concat([sine_sdt, cosine_sdt, windowed_sine_sdt, windowed_cosine_sdt], axis=-1)

    # first layer
    model = tf.add(tf.matmul(concatenated_sdt, weights['w1']), biases['b1'])
