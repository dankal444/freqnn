...
# network params
INC = 0.5
N_HIDDEN1 = int(2048 * INC)
N_HIDDEN2 = int(2048 * INC)
N_HIDDEN3 = int(2048 * INC)

N_HIDDEN_FRAMES1 = int(16 * INC)
N_HIDDEN_FRAMES2 = int(16 * INC)
...
def tensor_frames(tensor, frame_len, frame_rate):

    min_fundamental_idx = 380
    max_fundamental_idx = 1890

    indices_shifts = [0]
    multiplier = 2.0
    for i in range(100):
        harmonic_index = int(np.round(384 * np.log2(multiplier)))
        indices_shifts.append(harmonic_index)
        multiplier *= (i+3.0) / (i+2.0)
    indices_shifts = np.array(indices_shifts)

    frames_subtensors = []
    for fundamental_idx in range(min_fundamental_idx, max_fundamental_idx):
        # prepare harmonics indices
        indices_frame = indices_shifts + fundamental_idx

        # remove those out of analysis bounds
        indices_frame = indices_frame[indices_frame < len(ANALYSIS_FREQS)]

        frame_tensor = tf.gather(tensor, indices_frame, axis=-1)

        # first layer
        weight1layer = tf.Variable(tf.random_normal([len(indices_frame), N_HIDDEN_FRAMES1], 0.0, 0.0001))
        bias1layer = tf.Variable(tf.random_normal([N_HIDDEN_FRAMES1], 0.0, 0.0001))
        frame_tensor = tf.add(tf.matmul(frame_tensor, weight1layer), bias1layer)
        frame_tensor = tf.nn.relu(frame_tensor)

        # weight2layer = tf.Variable(tf.random_normal([N_HIDDEN_FRAMES1, N_HIDDEN_FRAMES2]))
        # bias2layer = tf.Variable(tf.random_normal([N_HIDDEN_FRAMES2], 0.0, 0.0001))
        # frame_tensor = tf.add(tf.matmul(frame_tensor, weight2layer), bias2layer)
        # frame_tensor = tf.nn.relu(frame_tensor)

        frames_subtensors.append(frame_tensor)

    tensor = tf.concat(frames_subtensors, axis=-1)
...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'w1': tf.Variable(tf.random_normal([1510 * 8, N_HIDDEN1], 0, 0.0001)),
    ...
    model = tensor_frames(magnitude_windowed_sdt, 384, 192)

    # first layer
    model = tf.add(tf.matmul(model, weights['w1']), biases['b1'])
