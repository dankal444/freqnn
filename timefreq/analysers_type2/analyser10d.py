...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'w1': tf.Variable(tf.random_normal([int(N_SAMPLES/2), N_HIDDEN1], 0, 0.0001)),
    ...
    # ))
    zeros = tf.constant(np.zeros(N_SAMPLES))
    fft = tf.abs(tf.fft(tf.cast(x_input_ph, tf.complex64)))# / tf.constant(N_SAMPLES / 10, dtype=tf.float32)

    #
    fft = tf.gather(fft, indices=np.arange(0, int(fft.shape[-1].value / 2)), axis=-1)
