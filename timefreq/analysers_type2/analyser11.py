...
N_HIDDEN_FRAMES1 = int(256 * INC)
N_HIDDEN_FRAMES2 = int(256 * INC)
...
def tensor_frames(tensor, frame_len, frame_rate):
    n_frames = int(float(tensor.shape[-1].value - frame_len) / frame_rate) + 1

    frames_subtensors = []
    for i_frame in range(n_frames):
        indices_frame = np.arange(i_frame * frame_rate, i_frame * frame_rate + frame_len)
        frame_tensor = tf.gather(tensor, indices_frame, axis=-1)

        # first layer
        weight1layer = tf.Variable(tf.random_normal([len(indices_frame), N_HIDDEN_FRAMES1], 0.0, 0.001))
...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'w1': tf.Variable(tf.random_normal([N_HIDDEN_FRAMES2 * 14, N_HIDDEN1], 0, 0.0001)),
        'w2': tf.Variable(tf.random_normal([N_HIDDEN1, N_HIDDEN2], 0, 0.001)),
        'w3': tf.Variable(tf.random_normal([N_HIDDEN2, N_HIDDEN3], 0, 0.001)),
        'out': tf.Variable(tf.random_normal([N_HIDDEN3, N_CLASSES], 0, 0.001)),
    }
    biases = {
    ...
    with tf.Session() as sess:
        ...
        for i_epoch in range(N_TRAINING_EPOCHS):
            ...
            if i_epoch % 1 == 0:
                import os
                train_data_directory_path = '/media/dankal/Y/bazy/audio/fake/f0analysis_ver2'
